...
/**
 * Extract article links from a DuckDuckGo search results page.
...
function extractSearchLinks(html) {
  ...
  // DuckDuckGo result links patterns — covers both /html/ and /lite/ responses
  const patterns = [
    // lite.duckduckgo.com: result links are plain <a href="https://..."> inside result rows
    /<a[^>]+href="(https?:\/\/(?!(?:www\.)?duckduckgo\.com)[^"]+)"[^>]*>([^<]{3,200})<\/a>/gi,
    // html.duckduckgo.com: class="result__a"
    /<a[^>]+class="[^"]*result__a[^"]*"[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi,
    // Generic fallback for any redirected result URL
  ...
  for (const pattern of patterns) {
    ...
    while ((m = pattern.exec(html)) !== null && links.length < 20) {
      ...
      if (url.includes('ad_domain')) continue
