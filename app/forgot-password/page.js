  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      stars.forEach(s => {
        ...
        ctx.fillStyle = `rgba(200,220,255,${Math.max(0, s.a + Math.sin(t * s.s + s.p) * 0.12)})`
        ctx.fill()
      })
    ...
    <div style={{ width:'100vw',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#03050c',fontFamily:'var(--font-mono)',position:'relative',overflow:'hidden' }}>
      ...
      <div style={card}>
        <div style={{ marginBottom: 32, textAlign: 'center' }}>
          ...
          <div style={{ fontSize:8,letterSpacing:4,color:'#3d7bd4' }}>RESET ACCESS</div>
        </div>
        <div style={{ height:1,marginBottom:28,background:'linear-gradient(90deg,transparent,rgba(61,123,212,0.4),transparent)' }} />

        {sent ? (
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:24,marginBottom:16,color:'#2a9e58' }}>◈</div>
            <div style={{ fontSize:10,color:'#7a9fbe',lineHeight:1.8,marginBottom:24 }}>
              Reset link sent to<br /><span style={{ color:'#c8e4ff' }}>{email}</span>
            </div>
            <a href="/login" style={{ display:'block',padding:'12px',border:'1px solid rgba(61,123,212,0.3)',color:'#7aaeee',fontSize:9,letterSpacing:3,textDecoration:'none',transition:'all 0.2s' }}
          ...
          <form onSubmit={handleReset} style={{ display:'flex',flexDirection:'column',gap:14 }}>
            <div style={{ fontSize:9,color:'#3a5878',letterSpacing:1,lineHeight:1.7,marginBottom:4 }}>
              ENTER YOUR EMAIL AND WE WILL SEND A RESET LINK.
            </div>
            <div>
              <label style={{ fontSize:8,letterSpacing:2,color:'#3a5878',display:'block',marginBottom:6 }}>EMAIL ADDRESS</label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="analyst@organisation.gov"
                style={{ width:'100%',background:'rgba(8,13,31,0.8)',border:'1px solid rgba(58,110,200,0.2)',borderRadius:2,color:'#ddeeff',fontSize:12,padding:'11px 14px',outline:'none',fontFamily:'var(--font-mono)',transition:'border-color 0.2s' }}
            ...
            <button type="submit" disabled={loading}
              style={{ width:'100%',marginTop:6,padding:'13px',background:'transparent',border:'1px solid rgba(100,160,240,0.35)',color:'#c8e4ff',fontSize:10,fontWeight:600,letterSpacing:4,transition:'all 0.2s',fontFamily:'var(--font-mono)',opacity:loading?0.7:1 }}
