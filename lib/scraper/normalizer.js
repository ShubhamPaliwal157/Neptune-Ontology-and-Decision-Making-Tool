...
export function extractSignalText(text = '', domain = '', topK = 30) {
  ...
  // Only filter if there's meaningful content to compress
  // (skip if already short AND no boilerplate detected)
  const hasBoilerplate = sentences.some(s => BOILERPLATE_RE.test(s))
  if (sentences.length <= topK && !hasBoilerplate) return text
  ...
  return topSentences.map(s => s.text).join(' ').replace(/ \. /g, ' ').replace(/\s+/g, ' ').trim()
}

function tokenizeSentences(text) {
  return text
    .replace(/\r?\n+/g, ' . ')                          // newlines → sentence boundary
    .replace(/([.!?])\s+(?=[A-Z])/g, '$1\n')            // punctuation + capital → split
    .split('\n')
    .flatMap(s => s.split(' . '))                        // also split on newline-converted boundaries
