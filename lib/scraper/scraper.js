...
const DEFAULT_CONCURRENCY  = 5   // parallel fetches at once
const DEFAULT_TIMEOUT_EACH = 25_000
...
async function buildUserSources(userSources, progress) {
  ...
  for (const raw of userSources) {
    ...
    if (src.type === 'keyword' || src.keyword) {
      // Convert keyword to a DuckDuckGo search URL
      const query = encodeURIComponent(src.keyword || src.url)
      result.push({
        id:     `user-kw-${slugify(src.keyword || '')}`,
        label:  `Keyword: ${src.keyword}`,
        domain: src.domain || 'unknown',
        type:   'web',
        url:    `https://html.duckduckgo.com/html/?q=${query}`,
        feed:   null,
      })
      progress(`  📌 Keyword source: "${src.keyword}"`)
      continue
    }
